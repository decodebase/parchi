...
export async function generateQRToken(payload: QRPayload, secret: string): Promise<string> {
  ...
  const payloadBytes_sign = new Uint8Array(encoder.encode(payloadB64));
  const sig = await crypto.subtle.sign(HMAC_ALGO, key, payloadBytes_sign);
...
export async function verifyQRToken(token: string, secret: string): Promise<VerifyResult> {
  ...
  try {
    ...
    const payloadBytes_verify = new Uint8Array(encoder.encode(payloadB64));
    const valid = await crypto.subtle.verify(HMAC_ALGO, key, sigBytes, payloadBytes_verify);
