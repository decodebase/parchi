...
export default async function AdminOrganisersPage({ searchParams }: PageProps) {
  ...
  const [profilesRes, appsRes] = await Promise.all([
    supabase
      ...
      .in("role", ["organiser", "scanner", "admin"])
  ...
  // Fetch applicant profile info separately
  const applicantIds = (appsData ?? []).map((a: any) => a.user_id).filter(Boolean);
  const { data: applicantProfiles } = applicantIds.length > 0
    ? await supabase.from("profiles").select("id, display_name, email").in("id", applicantIds)
    : { data: [] };
  ...
  // Fetch ID doc URLs and generate signed URLs in parallel
  ...
  try {
    ...
    // Collect all signed URL requests and fire them all in parallel
    const signedUrlTasks: Promise<void>[] = [];
    const entries: Record<string, typeof docsMap[string]> = {};

    for (const d of (appsWithDocs ?? []) as any[]) {
      entries[d.id] = { id_front_url: d.id_front_url, id_back_url: d.id_back_url };
      if (d.user_id && d.id_front_url) {
        signedUrlTasks.push(
          supabase.storage.from("id-documents").createSignedUrl(`${d.user_id}/id-front.jpg`, 3600)
            .then(({ data: s }) => { if (s?.signedUrl) entries[d.id].id_front_signed = s.signedUrl; })
        );
      }
      if (d.user_id && d.id_back_url) {
        signedUrlTasks.push(
          supabase.storage.from("id-documents").createSignedUrl(`${d.user_id}/id-back.jpg`, 3600)
            .then(({ data: s }) => { if (s?.signedUrl) entries[d.id].id_back_signed = s.signedUrl; })
        );
      }
    }
