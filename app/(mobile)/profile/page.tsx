...
export default function ProfilePage() {
  ...
  // Read auth state from the global store — AuthProvider already resolved this.
  // No need for a second onAuthStateChange listener on this page.
  const { user, profile, loading: authLoading } = useAuthStore();
  const [ticketCount, setTicketCount] = useState(0);
  const [ticketsLoaded, setTicketsLoaded] = useState(false);

  // Once auth is resolved and we have a user, fetch ticket count
  useEffect(() => {
    ...
    Promise.race([
      ...
      .then(({ count }: any) => setTicketCount(count ?? 0))
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="px-4 py-6 space-y-6">
        ...
        {/* Become Organiser — only for regular users */}
        {role === "user" && (
        ...
        <div className="space-y-2">
          {[
            { icon: ScanLine,        label: "Scanner Portal",  href: "/scan/portal", show: role === "scanner" },
